...
/**
 * Sortable calendar item within an account
 * Uses the calendar ID directly (unique within account context)
 */
export function SortableCalendarItem(props: SortableCalendarItemProps) {
  ...
  return (
    <div ref={sortable} class="rounded">
      <Show
        when={!sortable.isActiveDraggable}
        fallback={
          <div
            class="rounded bg-[#f0f0ee]"
            style={{ height: `${SIDEBAR.CALENDAR_ROW_HEIGHT}px` }}
          />
        }
      >
        <div
